...
function calculateStats(temperatures) {
...
function updateStats(stats) {
    ...
    document.getElementById('current-temp').textContent = `${stats.current.toFixed(1)}°C`;
...
function createChart(data) {
    ...
    const stats = calculateStats(data.temperature);
    updateStats(stats);

    // Mettre à jour le compteur
    document.getElementById('data-count').textContent =
        `${data.temperature.length} mesure${data.temperature.length > 1 ? 's' : ''} affichée${data.temperature.length > 1 ? 's' : ''}`;
}
...
// Log pour debug
console.log('Graphique température initialisé');